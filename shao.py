...
if __name__ == '__main__':
    ...
    size_range = [10*um,10*um]

    for n in range(0,100):
        t1 = time.time()
        # generate the random 3D location
        NUMBER = np.random.randint(low=50, high=51, dtype=int)
        Z_list = np.array(np.linspace(1*mm, 2.28*mm, 128))
        particles = particle_field(NUMBER,xyrange=size,z_list=Z_list,size_range=size_range)
        particles = particles.sort_values(by=['z'],ascending=False)
        particles.to_csv("shao_%d.csv"% n,index=False)
        holo = generate_holo_fromcsv2("shao_%d.csv"% n)
        plt.imsave("shao_%d.png" % n, holo, cmap='gray')
        t2 = time.time()
